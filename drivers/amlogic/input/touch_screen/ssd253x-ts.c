...
struct ChipSetting ssd253xcfgTable[]={							
	...
	{ 1,0x2E,	0x0B,	0x00},	//	25	
	{ 1,0x2F,	0x01,	0x00},	//	26	
	{ 1,0x30,	0x03,	0x00},	//	27	Win Begin
	{ 1,0x31,	0x06,	0x00},	//	28	Win End
	{ 1,0xD7,	0x04,	0x00},	//	29	
	{ 1,0xDB,	0x02,	0x00},	//	30	Intg Cap
	{ 2,0x33,	0x00,	0x00},	//	31	Min Area
	{ 2,0x34,	0x00,	0x2e},	//	32	Threshold	ÁéÃô¶ÈÉèÖÃ
	{ 2,0x35,	0x00,	0x20},	//	33	
	...
	{ 1,0x5B,	0x10,	0x00},	//	42	Charles change to 0x14, 12-03-26,
	{ 2,0x5E,	0x00,	0x40},	//	43	
	{ 1,0x40,	0xF0,	0x00},	//	44	
	{ 1,0x44,	0x01,	0x00},	//	45	
	{ 1,0x8A,	0x05,	0x00},	//	46	
	{ 1,0x65,	SSD253x_REPORT_COORD_ORIGIN,	0x00},	//	47		//change to Define,Charles 120321
	{ 2,0x66,	0xFF,	0xF0},	//	48
	{ 2,0x67,	0xFF,	0xF0},	//	49
	{ 2,0x7A,	0xFF,	0xFF},	//	50
	{ 2,0x7B,	0x00,	0x00},	//	51
	...
	{ 1,0xBB,	0x55,	0x00},	//	59
	{ 1,0xBC,	0x00,	0x00},	//	60
	{ 1,0x25,	SSD253x_REPORT_RATE_TIME,	0x00},	//	61		//Charles change to define
};
...
static int ssd253x_ts_probe(struct i2c_client *client,const struct i2c_device_id *idp)
{
	...
	if(pdata->power_gpio_no){
        gpio_request(pdata->reset_gpio_no, "TS_POWER");
	}
...
static int ssd253x_ts_resume(struct i2c_client *client)
{
	...
	struct ts_platform_data *pdata = client->dev.platform_data;
	if(pdata->power_gpio_no){
    	gpio_direction_output(pdata->power_gpio_no,1);
	}	
	deviceResume(client);
	ssd253x_timer_flag = 0;
	if(ssl_priv->use_irq) enable_irq(ssl_priv->irq);
...
static int ssd253x_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	mdelay(20);
	if(pdata->power_gpio_no){
    	gpio_direction_output(pdata->power_gpio_no,0);
	}		    
	return 0;
}
