...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if(state_count){
		rt_rest_cap = charger->ocv_rest_vol;
		rest_cap = charger->rest_vol;
		state_count ++;
		...
		/*
		if(ABS(rt_rest_cap - charger->ocv_rest_vol) > 5){
			printk("correct rdc\n");
	...
	charger->ocv_rest_vol = rt_rest_cap;
	...
	if((rt_rest_cap > 98) && (charger->rest_vol < 99) && (charger->bat_current_direction == 1)){
			if(axp_debug)
				DBG_PSY_MSG("%s->%d:(rt_rest_cap > 98) && (charger->rest_vol < 99)\n",__FUNCTION__,__LINE__);
	...
	if((rt_rest_cap > charger->rest_vol) && (charger->rest_vol < BATCAPCORRATE) && charger->bat_current_direction == 0){
		if(axp_debug)
			DBG_PSY_MSG("%s->%d(rt_rest_cap > %d) && (charger->rest_vol < %d) && charger->bat_current_direction == 0\n",__FUNCTION__,__LINE__,BATCAPCORRATE, BATCAPCORRATE);
	...
	if((rt_rest_cap < BATCAPCORRATE) && (charger->rest_vol > rt_rest_cap) && (charger->bat_current_direction == 0)){
		if(axp_debug)
			DBG_PSY_MSG("%s->%d(rt_rest_cap < %d) && (charger->rest_vol > %d) && (charger->bat_current_direction == 0)\n",__FUNCTION__,__LINE__,BATCAPCORRATE,BATCAPCORRATE);
...
/*  DBG_PSY_MSG("pmu_pekoff_time = %d\n",pmu_pekoff_time);
  ...
  */
  ...
  if(((val1 >> 7) & 0x1) == 0){
	...
	DBG_PSY_MSG("axp_set_basecap(charger,rt_rest_cap) = %d\n",charger->rest_vol);
...
#if defined (CONFIG_AXP_CHGCHANGE)
    if(axp_cfg_board->pmu_suspend_chgcur >= 300000 && axp_cfg_board->pmu_suspend_chgcur <= 1800000){
    tmp = (axp_cfg_board->pmu_suspend_chgcur -200001)/100000;
    charger->chgcur = tmp *100000 + 300000;
...
#if defined (CONFIG_AXP_CHGCHANGE)
    if(axp_cfg_board->pmu_shutdown_chgcur >= 300000 && axp_cfg_board->pmu_shutdown_chgcur <= 1800000){
    	tmp = (axp_cfg_board->pmu_shutdown_chgcur -200001)/100000;
    	charger->chgcur = tmp *100000 + 300000;
    	axp_update(charger->master, AXP20_CHARGE_CONTROL1, tmp, 0x0F);
    }
#endif
