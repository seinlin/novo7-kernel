...
extern struct clk *clk_get_sys_locked(const char *dev_id, const char *con_id);
...
static int clk_set_rate_clk81(struct clk *clk, unsigned long rate)
{
    ...
    father_clk = clk_get_sys_locked("clk_misc_pll", NULL);
...
static int clk_set_rate_a9_clk(struct clk *clk, unsigned long rate)
{
    ...
    father_clk = clk_get_sys_locked("clk_sys_pll", NULL);
	divider = 0;
...
static int clk81_freq_setting_flag = 0;
static int pll_off_flag = 0;

#define MAX_FREQ_LEVEL 1

LIST_HEAD(clk81_list_head);
...
int regist_clk81_client(char* name, clk81_note_fn* pre_fun, clk81_note_fn* post_fun, 
					clk81_note_fn* check_fun, void* param)
{
	struct clk81_client* client = NULL;
	client = kzalloc(sizeof(struct clk81_client),GFP_KERNEL);
	if (!client) {
		printk(KERN_INFO "no memory clk81_client regestion failed\n");
		return - 1;
	}
	client->name = name;
	client->param = param;
	client->pre_change_fun = pre_fun;
	client->post_change_fun = post_fun;
	client->check_client_ready = check_fun;
	
	list_add(&client->list, &clk81_list_head);
	
	return 0;
}
EXPORT_SYMBOL(regist_clk81_client);

int unregist_clk81_client(char* name)
{
	struct list_head *p;
	struct clk81_client* client = NULL;
	list_for_each(p, &clk81_list_head) {
		client = list_entry(p, struct clk81_client, list);
		if (client && !strcmp(client->name, name)) {
...
EXPORT_SYMBOL(unregist_clk81_client);

static void pre_change_clk81(void)
{
	struct list_head* p;
	struct clk81_client* client = NULL;
	list_for_each(p, &clk81_list_head) {
		client = list_entry(p, struct clk81_client, list);
		if (client && client->pre_change_fun) 
			client->pre_change_fun(client);
	}
}

static void post_change_clk81(void)
{
	struct list_head* p;
	struct clk81_client* client = NULL;
	list_for_each(p, &clk81_list_head) {
		client = list_entry(p, struct clk81_client, list);
		if (client && client->post_change_fun) 
			client->post_change_fun(client);
	}
}	

static int check_all_client_ready(void)
{	
	struct list_head* p;
	struct clk81_client* client = NULL;
	list_for_each(p, &clk81_list_head) {
		client = list_entry(p, struct clk81_client, list);
		if (client && client->check_client_ready) {
			if (client->check_client_ready(client) < 0)
				return -1;
		}
	}
	return 0;
...
static int clk81_pll_prepare(unsigned crystal_freq, unsigned  out_freq)
{
    int n, m, od;
...
static ssize_t  clk81_level_store(struct class *cla, struct class_attribute *attr, char *buf,size_t count)
{
	...
	if(sscanf(buf, "%d", &val) == 1) {
		...
		if ((0 != strncmp("", get_vout_mode_internal(), 5)) && 
		    (0 != strncmp("panel", get_vout_mode_internal(), 5))) {
		    val = 1;
		}
		...
		if (pll_off_flag && val == 1) {
			SET_CBUS_REG_MASK(HHI_OTHER_PLL_CNTL, (0 << 15)); // other pll on
			pll_off_flag = 0;
			if  (clk81_pll_prepare(crystal_freq, level_freq[1]*4) != 0) {
		...
		while(check_all_client_ready() < 0) //waiting clients ready
			msleep(10);
			
		clk81_freq_setting_flag = 1;
		new_clk81_freq_level = val;
	
		while(clk81_freq_setting_flag) //waiting freq setting
			msleep(10);
...
static int __init clk81_dfs_init(void)
{
	int ret;
