...
#include "board-m3-reff06-mid7306ce.h"
...
/******************************
*WL_Power_EN	-->unusefull
*WL_REG_ON		-->GPIOC_8
*WIFI_32K		-->GPIOC_15(CLK_OUT1)
*WIFIWAKE(WL_HOST_WAKE)-->GPIOX_15(unusefull)
*******************************/
...
void extern_wifi_enable(int enable)
{
	if(enable){
...
EXPORT_SYMBOL(extern_wifi_enable);
...
{
	//set WIFI Power Pin GPIOA_4 out 
...
//    CLEAR_CBUS_REG_MASK(PREG_PAD_GPIO0_EN_N, (1<<4));  //GPIOA_4
...
#define GPIO_WIFI_HOSTWAKE  ((GPIOX_bank_bit0_31(15)<<16) |GPIOX_bit_bit0_31(15))
void sdio_extern_init(void){
	printk("sdio_extern_init !\n");
#if defined(CONFIG_BCM40181_HW_OOB) || defined(CONFIG_BCM40181_OOB_INTR_ONLY)
    printk("40181 set oob ok !\n");
    CLEAR_CBUS_REG_MASK(PAD_PULL_UP_REG4, (1<<11));
...
{
	if(get_current_vmode() != VMODE_LCD) //if !panel, return 1 to mute spk		
		return 1;
...
static struct ts_platform_data ts_pdata = {
	.irq_no 			= FT_IRQ,
	.reset_gpio_no		= ((GPIOD_bank_bit0_9(5)<<16) |GPIOD_bit_bit0_9(5)),
...
static struct android_usb_platform_data android_usb_pdata = {
       ...
       .product_id     = 0x0c01,
       .version        = 0x0100,
       .product_name   = "MID7306CE",
       .manufacturer_name = "Henag",
...
static int __init aml_i2c_init(void)
{
    ...
    pmu_battery_rdc = 94;//70;//BATRDC;
    pmu_battery_cap = 3700;//7800;//BATTERYCAP;
